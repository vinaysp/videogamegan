...
def generate_images(generator, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir+str(time.time())+"_GENERATEDimage.jpg",bbox_inches='tight',pad_inches=0)
    #plt.show()
...
save_img_batch(sample_from_dataset(batch_size, image_shape, data_dir = data_dir),img_save_dir+"_12TRAINimage.jpg")
"""

#Generating GIF from PNG
images = []
all_data_dirlist = list(glob.glob(img_save_dir+"*_image.jpg"))
...
Alternate way to convert PNG to GIF (ImageMagick):
    >convert -delay 10 -loop 0 *_image.jpg animated.gif
"""
